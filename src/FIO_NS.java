...
public class FIO_NS {
	// The local address
//	private int localAddr = PropertyHelper.getInt("local.addr", 0xDAAA);
	private int localAddr = 0xBABE;
	// the remote address, address of the receiver
//	private int remoteAddr = PropertyHelper.getInt("remote.addr", 0xBABE);
	private int remoteAddr = 0xDAAA;
	// the panID to use
	...
	private void startTransmitter(final FrameIO fio) {
		new Thread() {
			...
			public void run() {
				...
				testFrame.setPayload("Test".getBytes());
